...
def probe_tool(binary: str, version_flag: str = "--version") -> tuple[bool, str]:
    ...
    if shutil.which(binary):
        try:
            r = subprocess.run([binary, version_flag], capture_output=True, timeout=5, text=True)
    ...
    if IS_WINDOWS and has_wsl():
        try:
            r = subprocess.run(["wsl", binary, version_flag], capture_output=True, timeout=5, text=True)
            if r.returncode == 0:
                ver = (r.stdout or r.stderr).splitlines()[0] if (r.stdout or r.stderr) else ""
